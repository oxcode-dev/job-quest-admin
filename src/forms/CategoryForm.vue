...
const form = ref(props.category && props.category.id ? JSON.parse(JSON.stringify(props.category)) : {
    name: '',
})
...
const submit = async() => {
    if(form.value.name) {
        ...
        if(props.category && props.category.id) {
            await updateData('categories', form.value, props.category.id)
        }
        else {
            await addData('categories', form.value)
        }
